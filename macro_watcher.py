...
def _gemini_factcheck(gkey, brief, mdetail=""):
    ...
    _prompt = (
        ...
        "②검색으로 확인 안 되는 주장은 '❓미확인'으로 명시(지어내지 마). "
        "③뉴스만 있고 수주·계약·공시 근거 없는 테마는 '추격주의'로 강등. "
        "④[실측 시장데이터]와 브리핑이 다르면 실측을 정답으로 간주.\n"
        "★핵심 안전규칙: 검색 도구를 못 쓰거나 검색 결과가 없으면, 뉴스의 '시점·최신성·진위'를 절대 단정하지 마라. "
        "특히 네가 학습기억으로 '과거 뉴스다/몇년도 일이다'라고 추측해 실제 오늘 재료를 가짜로 몰지 마라(과거 사건이 지금 재발할 수도 있음). "
        "확인 불가는 반드시 '❓미확인(개장 후 확인)'으로만 표기하고, 오직 [실측 시장데이터]와 '직접' 충돌하는 수치만 교정하라.★\n\n"
        f"[실측 시장데이터]\n{mdetail}\n\n[1차 브리핑]\n{brief}\n\n"
        "[출력·텔레그램용·간결·이모지]\n"
        "🔎 팩트체크: (틀린/과장된 수치·주장 2~4개를 '주장→실제(또는 ❓미확인)'로 교정)\n"
