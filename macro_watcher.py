...
def _price_turnover_vol(token, key, secret, code, mrkt="J"):
    """[V26.7] _price_and_turnover와 동일 API(inquire-price) 한 번으로 누적거래량(주식수)까지 함께
    추출 — (px, chg, turnover원, vol주) 또는 (None,None,None,None). 대체종배가 "거래대금(원화)
    ...
    try:
        ...
        if isinstance(o, dict) and o:
            return (_to_int(o.get("stck_prpr")),
                    float(str(o.get("prdy_ctrt", 0)).replace(",", "") or 0),
                    _to_int(o.get("acml_tr_pbmn")), _to_int(o.get("acml_vol")))
    except Exception:
        pass
...
_DART_POS = ("공급계약체결", "단일판매", "수주", "기술이전", "특허권취득", "품목허가", "임상시험결과",
             "자기주식취득결정", "무상증자결정", "시설투자", "타법인주식및출자증권취득")
# [V26.3] 실전 갭상승 역분석(9/18)에서 가온전선(+6.0%)이 "장마감 후 대규모 미국 설비투자 공시로
# 전날 감지 가능했다"고 나왔는데, 당시 _DART_POS에 시설투자 계열 키워드가 아예 없어서
# _neg/_pos/_perf/_is_agm 전부 False → 아래 `continue`로 조용히 통째로 스킵되고 있었음
# (다른 재료 유형은 다 있는데 이것만 빠져있던 사각지대). 금액 기반 등급 세분화는 하지 않음
# — _dart_material_grade의 계약금액 파싱(_contract_detail)이 공급계약 서식 전용이라, 시설투자
# 서식에 그대로 재사용하면 파싱 실패 위험 → 일단 뉴스등급 기반 기본 판정(🟢보통재료)으로 유입만 시킴.
...
def check_holdings(token, key, secret, now_kst, state, token_tg, chat_id):
    ...
    # [V25.58] 버그수정 — 예전엔 NXT거래가능 여부를 _nxt_tradable() 1회 판정으로 캐싱했는데, 그 1회
    # 판정 시점에 하필 시세가 비는 경계(15:30~16:00 등)와 겹치면 오판정이 그대로 하루종일 굳어버려
    # (서산·머큐리가 실제로 54회 연속경보까지 간 사례로 확인) 이번엔 '오늘 NX에서 유효가를 한번이라도
    # 실제로 본 적 있는가'라는 누적 증거 기반으로 바꿈 — 1회 판정에 기대지 않고 계속 자기교정된다.
    ...
    state["holdings_nxt_seen"] = _nxt_seen
...
# ── [대체종배] NXT 애프터마켓 거래량이 당일 정규장 거래량을 압도하는 종목 기반 NXT 전용 픽 ──
...
#   "NXT 거래량이 당일 총거래량을 압도"였던 것.
#   [V26.6] "NXT거래대금이 그 종목 당일 정규장 거래대금 대비 몇 배인가"(원화 금액 비율)로 1차
#   재설계했으나, 사용자가 준 9/23 엑셀 원본 데이터로 에스투더블유를 직접 대조한 결과 거래대금
#   비율은 0.43배(기준 미달)인데 거래량(주식수) 비율은 3.52배(기준 충족)로 서로 다른 결론이 남 —
#   가격×거래량인 거래대금은 등락률에 따라 왜곡되고, 사용자가 실제로 말한 건 "거래량"(주식수) 그
#   자체였음.
    ...
    while True:
        ...
        try:
            ...
            snap = {
                ...
                "nxt_premium_pick": [],                # 대체종배(NXT거래량/정규장 당일거래량 비율 압도 종목)
            }
